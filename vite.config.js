        ...
        tailwindcss(),
        {
            ...
            configureServer(server) {
                ...
                function titleFromUrlPath(urlPath) {
                    ...
                    return last.split(/[-_]+/g).filter(Boolean).map((w) => w[0].toUpperCase() + w.slice(1)).join(' ');
                }
                ...
                async function listHtmlFiles(dir) {
                    ...
                    for (const ent of entries) {
                        ...
                        if (ent.isDirectory()) out.push(...(await listHtmlFiles(full)));
                        else if (ent.isFile() && ent.name.toLowerCase().endsWith('.html') && !ent.name.startsWith('_')) out.push(full);
                    }
                    return out;
                }
                ...
                server.middlewares.use(async (req, res, next) => {
                    try {
                        ...
                        const html = await renderPage({
                            layoutsDir, pagePath, componentsDir, siteConfig,
